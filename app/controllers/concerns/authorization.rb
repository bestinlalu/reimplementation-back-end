...
module Authorization
  ...
  def current_user_is_assignment_participant?(assignment_id)
    if user_logged_in?
      return AssignmentParticipant.exists?(parent_id: assignment_id, user_id: current_user.id)
    end
    false
  end
  ...
  def current_user_created_bookmark_id?(bookmark_id)
    user_logged_in? && !bookmark_id.nil? && Bookmark.find(bookmark_id.to_i).user_id == current_user.id
  ...
  # will return a user even if current_user has been explicitly cleared out
  # because it is "sticky" in that it uses "@current_user ||= current_user"
  ...
  def current_user_ancestor_of?(user)
    return current_user.recursively_parent_of(user) if user_logged_in? && user
    false
  end
